...
export async function fetchPlaylistInfo(playlistId: string): Promise<PlaylistInfo> {
  ...
  if (!contentType || !contentType.includes("application/json")) {
    throw new Error(`API server is temporarily unavailable or returned an invalid response (Status: ${response.status}, Content-Type: ${contentType}). Please try again.`);
  }
...
export async function fetchPlaylistItems(playlistId: string): Promise<PlaylistItem[]> {
  ...
  try {
    ...
    console.warn('Error fetching playlist items:', error);
    throw error;
  }
}
